...
def process_results(results, gray_frame_bgr, base_color):
    ...
    :param results: Resultados del modelo YOLO.
    :param gray_frame_bgr: Frame en formato BGR.
    :param base_color: Color base para dibujar las curvas.
    :return: Frame procesado con las curvas dibujadas.
    """
        ...
        if num_valid_masks > 0:
            ...
            for i_orig in closest_original_indices:
                ...
                if len(filtered_points) > 20:
                    # Generate a unique color for each index by modifying the base color
                    color = (
                        (base_color[0] + i_orig * 30) % 256,  # Adjust red channel
                        (base_color[1] + i_orig * 50) % 256,  # Adjust green channel
                        (base_color[2] + i_orig * 70) % 256   # Adjust blue channel
                    )
...
while cap.isOpened():
    ...
    results = model(gray_frame_bgr, show=False, show_boxes=False)
    # print(results)
