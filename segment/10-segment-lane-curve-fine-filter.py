...
def fit_polynomial_ransac(points, degree=2, residual_threshold=5):
    ...
    # Dynamically set min_samples
    min_samples = max(2, len(points) // 2)
...
def draw_curve(points, color, gray_frame_bgr,
    ...
    print(f"Filtered points: {filtered_points}")  # Debugging line to check filtered points

    if len(filtered_points) < 2:
        print("Not enough points to process. Skipping curve drawing.")
    ...
    mask, model = fit_polynomial_ransac(pts, degree=2, residual_threshold=7)

    inliers = pts[mask]
    if len(inliers) < 2:
        print("Not enough inliers to draw a curve. Skipping.")
        return gray_frame_bgr
